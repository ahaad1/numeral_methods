...
def write_xlsx(arr: list, row: int, col: int, sheet, message: str = None):
    ...
    :param arr: Двумерный массив (список списков) или одномерный массив (список), который нужно записать.
    ...
    """
    try:
        # Преобразование одномерного массива в двумерный формат
        if isinstance(arr[0], (int, float, str)):  # Проверка на одномерный массив
            arr = [[x] for x in arr]  # Преобразуем в столбец
